# this file work as: 1. loading the server side model.
# 2. receiving data.
# 3. finish the computation for the received data(inference).
# 4. send the output back to the edge.
# 5. moving buffer based on performance
...
class LLaMA:
    ...
    @staticmethod
    def build(checkpoints_dir: str, tokenizer_path: str, model: [], load_model: bool, max_seq_len: int,
    ...
    def eval(self, input, freq, cur_pos, splitting_point):
        server_compute_time = 0
        with torch.no_grad():
            startTime = time.time()
            logits = self.model[splitting_point + 1].forward(input, freq, cur_pos)
            for i in range(splitting_point + 2, len(self.model)):
                logits = self.model[i].forward(logits, freq, cur_pos)
            endTime = time.time()
            server_compute_time = server_compute_time + (endTime - startTime)

        print('server compute time: ', server_compute_time)
        return logits
...
if __name__ == '__main__':
    ...
    server_communication_time = 0
    server_communication_time2 = 0
    while True:
    #    message = client_socket.recv(1024)
        #message = input('Me > ')

        #message = client_socket.recv(1024)
        #logits = pickle.loads(message)
        #print('socket received >', logits)


        # receive frequency
        startTime = time.time()
        newxlen = client_socket.recv(4)
        ...
        while 1:
            ...
            newx += packet
        freq = pickle.loads(newx)
        #print('socket received (freq) >', freq)
        ...
        while 1:
            ...
            size = size - len(packet)
            newx += packet
        cur_pos = pickle.loads(newx)
        #print('socket received (position) >', cur_pos)
        ...
        while 1:
            ...
            size = size - len(packet)
            newx += packet
        logit = pickle.loads(newx)
        #print('socket received (logit) >', logit)
        endTime = time.time()
        server_communication_time = server_communication_time + (endTime - startTime)

        splitting_point = args.splitting_point
        lm_logit = model.eval(logit, freq, cur_pos, splitting_point)

        startTime2 = time.time()
        newx = pickle.dumps(lm_logit)
        newxlen = len(newx)

        newxlen = newxlen.to_bytes(4, byteorder='big')
        client_socket.sendall(newxlen)
        client_socket.sendall(newx)
        endTime2 = time.time()
        server_communication_time2 = server_communication_time2 + (endTime2 - startTime2)

        print('server communication time: ', server_communication_time)
        print('server communication time2: ', server_communication_time2)
...
server = ssl.wrap_socket(
    ...
    server, server_side=True, certfile="/home/yichun/.ssh/authorized_keys"
)
