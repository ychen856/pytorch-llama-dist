...
def prepare_calibration_input(models, dataloader, device):
    use_cache = models[0].config.use_cache
    models[0].config.use_cache = False
    #layers = models[1].model.layers

    # dev = model.hf_device_map["model.embed_tokens"]
    '''if "model.embed_tokens" in models[0].hf_device_map:
        print('yes')
        device = models[0].hf_device_map["model.embed_tokens"]
    else:
        print('no')'''
    print('device: ', device)
    dtype = next(iter(models[0].parameters())).dtype
    #inps = torch.zeros((128, model.seqlen, model.config.hidden_size), dtype=dtype, device=device)
    inps = torch.zeros((128, 1024, models[0].config.hidden_size), dtype=dtype, device=device)
    inps.requires_grad = False
...
def prune_wanda_outlier(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    print("loading calibdation data")
    dataloader, _ = get_loaders("c4",nsamples=128,seed=0,seqlen=1024,tokenizer=tokenizer)
    print("dataset loading complete")
    with torch.no_grad():
