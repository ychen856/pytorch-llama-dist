...
def prepare_calibration_input(model, dataloader, device):
    ...
    inps = torch.zeros((128, model.args.max_seq_len, 256), dtype=dtype, device=device)
    inps.requires_grad = False
...
def prune_mag_outlier(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    print("loading calibdation data")
    #dataloader, _ = get_loaders("c4", nsamples=5, seed=0, seqlen=20, tokenizer=tokenizer)
    allow_cuda = False
...
def prune_wanda_outlier_structure_lbl(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ##### calucalte outlier ratio
    all_layer_ratio = []
    use_cache = model.config.use_cache
    model.config.use_cache = False
