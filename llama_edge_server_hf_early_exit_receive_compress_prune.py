    ...
    if type == 3:   #reallocate model
        ...
        for checkpoint in checkpoints:
            ...
            if checkpoint_idx >= len(models):
                ckpt_path = checkpoint
                checkpoint_list.append(torch.load(ckpt_path, map_location="cpu"))
            elif checkpoint_idx > end_idx_buff or models[checkpoint_idx] is None:
                #print('yaaay')
                ckpt_path = checkpoint
                checkpoint_list.append(torch.load(ckpt_path, map_location="cpu"))
...
def get_server_statistic_from_q():
    while not http_sender_gateway.returning_queue.empty():
        ...
        performance_data_store.server_comp_statistics = (start_idx, server_comp_time)
        performance_data_store.comm_statistics = rtt - server_comp_time
        ...
        #while outgoing_queue.qsize() < 3 and input_queue.qsize() > 0:
            ...
            if incoming_queue.qsize() > 0: #and calculate_opt.incoming_count + 2 >= calculate_opt.outgoint_count:
                ...
                if start_idx == 0:
                    ...
                    outgoing_queue_forward.put([start_idx, pruned_feature_vector, ids, mask, idx, 0, start_idx])
