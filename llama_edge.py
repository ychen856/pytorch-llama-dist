...
class LLaMA:
    ...
    def eval(self, input, splitting_point, soc):
        ...
        client_compute_time = 0
        client_communication_time = 0
        for cur_pos in cur_iterator:
            is_early_exit = False
            ...
            with torch.no_grad():
                ...
                for i in range(1,  splitting_point + 1):
                    #print(i)
                    logits = self.model[i].forward(logits, freq, cur_pos)
                    if i == splitting_point + 1:
                        ...
                        if torch.max(probs_sort).item() > top_p:
                            ...
                            break
                endTime = time.time()
                client_compute_time = client_compute_time + (endTime - startTime)

                startTime2 = time.time()
                if is_early_exit == False:
                    # send frequency
                    newx = pickle.dumps(freq)
                    newxlen = len(newx)
                    ...
                    newx = b''
                    while 1:
                        packet = soc.recv(size)
                        if not packet:
                            break
                        size = size - len(packet)
                        newx += packet
                    logits = pickle.loads(newx)
                    #print('socket received (output) >', logits)
                endTime2 = time.time()
                client_communication_time = client_communication_time + (endTime2 - startTime2)
        ...
        print('client compute time: ', client_compute_time)
        print('client communication time: ', client_communication_time)
        return logits
...
if __name__ == '__main__':
    ...
    max_prompt_len = max(len(prompt) for prompt in testloader)
    # Make sure the prompt length is not larger than the maximum sequence length
    ...
    tokens = torch.full((batch_size, total_len), pad_id, dtype=torch.long, device=device)
    for k, t in enumerate(testloader):
        # Populate the initial tokens with the prompt tokens
        tokens[k, : len(t)] = torch.tensor(t, dtype=torch.long, device=device)
    ...
    for i in range(0, nsamples, bs):
        ...
        lm_logits = model.eval(inputs, splitting_point, soc)
