...
def eval_ppl_sep_hf(models, tokenizer, start_idx, end_idx, device=torch.device("cuda:0")):
    seqlen = 1024
    # Set dataset
    ...
    with torch.no_grad():
        ppl = eval_ppl_wikitext_sep_hf(models, testloader, start_idx, end_idx, 1, device)
    return ppl


def eval_ppl_wikitext_sep_hf(models, testenc, start_idx = 0, end_idx = 34, bs=1, device=None):
    seqlen = 1024
    # Get input IDs
